...
Dementsprechend sind folgende Untersuchungen laut S3-Leitlinie @DeutscheKrebsgesellschaftEtAl2019 notwendig: 
...
- Einnahme von Brivudin in der **letzte 4 Wochen**.
(Nach Abschluss der Brivudin-Behandlung muss eine mindestens vierwöchige Wartezeit,
eingehalten werden, bevor die Behandlung mit einem Fluoropyrimidin begonnen werden kann.) @Aerzteblatt2012
...
Sowohl die Kurzzeitbestrahlung (5 × 5 Gy) als auch die konventionell fraktionierte präoperative Radiochemotherapie haben spezifische Vor- und Nachteile in der Behandlung des lokal fortgeschrittenen Rektumkarzinoms [@SauerEtAl2004; @BahadoerEtAl2021]. Die Kurzzeitbestrahlung ist effizient und in der Regel gut verträglich, lässt jedoch offene Fragen zur Langzeitwirksamkeit [@BujkoEtAl2006]. Die Radiochemotherapie führt zu einem besseren Downstaging, ist aber mit höherer akuter Toxizität assoziiert [@JinEtAl2022].

Diese Unsicherheit spiegelt sich auch in der aktuellen deutschen S3-Leitlinie @DeutscheKrebsgesellschaftEtAl2019 wider, die einen Empfehlungsgrad 0 ausspricht:
...
Erstmals kommt hier eine intensivierte Radiochemotherapie mit 5-FU und Oxaliplatin zum Einsatz. Primäres Ziel bleibt der organerhaltende Therapieansatz bei optimaler lokaler Tumorkontrolle.
...
```{r trials-table, echo=FALSE, message=FALSE, warning=FALSE}
...
#  if (knitr::is_html_output()) {
    studien_table <- data_clean %>%
      ...
      tab_source_note(
      source_note = "cCR = clinical complete response, CRT = chemoradiotherapy, TME = Total mesorectal excision, LV = Leucovorin, OS = Overall survival, DSF = disease-free survival, pCR = pathological complete response, SCRT = Short-course radiation therapy (25 Gy/5f)."
    ) %>%
  opt_row_striping()
    
gt_theme_nytimes(studien_table)
